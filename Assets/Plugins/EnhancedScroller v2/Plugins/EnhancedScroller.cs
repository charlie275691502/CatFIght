...
namespace EnhancedUI.EnhancedScroller
{
    ...
    /// <summary>
    /// This delegate is called to create a cell view for the first time
    /// </summary>
    /// <param name="prefab">The cell view prefab to create</param>
    public delegate EnhancedScrollerCellView CellViewInstantiater(EnhancedScrollerCellView prefab);
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        /// </summary>
        public CellViewInstantiater cellViewInstantiater;
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab)
        {
            ...
            if (cellView == null)
            {
                ...
                if (cellViewInstantiater != null)
                {
                    cellView = cellViewInstantiater(cellPrefab);
                }
                else
                {
                    cellView = Instantiate<EnhancedScrollerCellView>(cellPrefab);
                }
        ...
        public virtual void ReloadData(float scrollPositionFactor = 0)
        {
            _reloadData = false;
        ...
        public void Awake()
        {
            // We don't want to rely on unity to call the awake function
            // Instead, we move all the code writen here into _Initialize()
            // so that we ensure EnhancedScroller class will be initialized before the Reload function at SdoricaEnhancedScroller.Reload()
        }
        ...
        void OnEnable()
        {
            // We don't want to rely on unity to call the Enable function
            // Instead, we move all the code writen here into _Register() and call it after _Initialize()
        }

        void OnDisable()
        {
            // We don't want to rely on unity to call the Disable function
            // Instead, we move all the code writen here into _Register() and call it in _Initialize()
        }
